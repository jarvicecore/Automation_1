...
_CHECKS = {
    "secrets": lambda root, max_file_size_mb: check_secrets(root),
    "large-files": lambda root, max_file_size_mb: check_large_files(
        root, max_size_mb=max_file_size_mb
    ),
    "hygiene": lambda root, max_file_size_mb: check_repo_hygiene(root),
}


def _run_checks(root: Path, max_file_size_mb: float, checks: list[str]) -> list[Finding]:
    findings: list[Finding] = []
    for name in checks:
        findings += _CHECKS[name](root, max_file_size_mb)
    return findings
...
def main(argv: list[str] | None = None) -> int:
    ...
    findings = _run_checks(root, max_file_size_mb=args.max_file_size_mb, checks=args.checks)
