...
def test_exclude_suppresses_findings_under_prefix(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    ...
    exit_code = main([str(tmp_path), "--checks", "secrets", "--exclude", "tests"])

    assert exit_code == 0
    assert "no findings" in capsys.readouterr().out.lower()


def test_exclude_does_not_suppress_findings_outside_prefix(tmp_path: Path) -> None:
    fixtures = tmp_path / "tests"
    fixtures.mkdir()
    (fixtures / "test_secrets.py").write_text('AWS_KEY = "AKIAABCDEFGHIJKLMNOP"\n')
    (tmp_path / "config.py").write_text('AWS_KEY = "AKIAABCDEFGHIJKLMNOP"\n')
